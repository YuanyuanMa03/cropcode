...
MCP 工具在 CropCode 中的命名格式为 `mcp__<服务名>__<工具名>`，例如 `mcp__github__search_code`。

## 配置 MCP 服务器

编辑 `~/.cropcode/settings.json`，添加 `mcpServers` 字段：
...
| 字段      | 类型     | 必填 | 说明                                                                                                                   |
| --------- | -------- | ---- | ---------------------------------------------------------------------------------------------------------------------- |
| `command` | string   | 是   | MCP 服务器的可执行文件路径或命令（如 `npx`、`node`、`python`）。当命令是 `npx` 时，CropCode 会自动在参数前补充 `-y`。 |
...
让 CropCode 直接操作 GitHub 仓库（搜索代码、管理 Issue/PR、读写文件等）：
...
以下是一个配置了 GitHub 和 Playwright 两个 MCP 服务器的完整 `~/.cropcode/settings.json`：
...
配置完成后，启动 `cropcode`，在聊天中输入 `/mcp` 即可查看所有已配置的 MCP 服务器状态以及每个服务器提供的工具列表。
...
3. 运行 `cropcode` 的终端是否有网络访问权限

### 工具不显示

1. 确认 `settings.json` 中的 `mcpServers` 字段格式正确
2. 启动 cropcode 后使用 `/mcp` 查看服务器状态

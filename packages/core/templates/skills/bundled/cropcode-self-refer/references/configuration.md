...
CropCode 使用 `settings.json` 设置文件进行持久化配置，支持两个层级的存放位置：

| 文件类型     | 位置                                 | 作用范围                                              |
| ------------ | ---------------------------------- | ---------------------------------------------------- |
| 用户设置文件 | `~/.cropcode/settings.json`         | 适用于当前用户的所有 CropCode 会话。                      |
| 项目设置文件 | `项目根目录/.cropcode/settings.json` | 仅在该特定项目中运行 CropCode 时生效。项目设置会覆盖用户设置。 |
...
> 当 `command` 为 `npx` 时，CropCode 会自动在参数前补充 `-y`。
...
优先级层级 (由低到高)
1. settings.json 外层的 env：这是针对整个工具及其所有子进程的通用配置（全局变量）。可被外层环境变量覆盖，但环境变量KEY会移除`CROPCODE_`前缀。
...
4. 系统环境变量: `CROPCODE_API_KEY=abc123 cropcode`
...
6. 系统环境变量: `CROPCODE_THINKING_ENABLED=true cropcode`
...
4. 系统环境变量: `CROPCODE_WEBHOOK=... cropcode`

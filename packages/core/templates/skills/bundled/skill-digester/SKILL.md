---
name: skill-digester
description: Reviews and improves another CropCode skill's SKILL.md description field against the Agent Skills description-field rules. Use when the user asks to "digest" a skill, including requests like "digest the pdf skill" or "消化 pdf 技能".
---

# Skill Digester

Use this skill to review and optionally rewrite the `description` field of another CropCode skill.
...
1. Identify the target skill from the user's request.
   ...
   - Locate the skill by running the bundled Node script from this skill directory:
     ...
     ```bash
     node ~/.cropcode/skills/skill-digester/scripts/find-skill.js "<skill-name-or-path>" "<project-root>"
     ```
   ...
   - The script searches the same roots CropCode CLI scans, in priority order:
     1. Project native skills: `./.cropcode/skills/<folder>/SKILL.md`
     2. Project interoperable skills: `./.agents/skills/<folder>/SKILL.md`
     3. User native skills: `~/.cropcode/skills/<folder>/SKILL.md`
     4. User interoperable skills: `~/.agents/skills/<folder>/SKILL.md`
   - Treat `./` as the current CropCode project root only; do not scan parent directories unless the running project root is changed.
   - The script resolves each candidate's skill name the way CropCode does: use the trimmed frontmatter `name` when present, otherwise use the folder name with underscores converted to hyphens.
   ...
   - `digestTarget.path` always points to the same scope's native CropCode root:
     - Project sources from `./.cropcode/skills` or `./.agents/skills` digest to `./.cropcode/skills/<folder>/SKILL.md`.
     - User sources from `~/.cropcode/skills` or `~/.agents/skills` digest to `~/.cropcode/skills/<folder>/SKILL.md`.
   ...
   - If the script returns no match, state that the skill was not found in CropCode's scanned skill roots and use `AskUserQuestion` to ask whether the user wants to try another name.

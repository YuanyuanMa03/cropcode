...
test("WebSearch uses the default API when no script is configured", async () => {
  ...
  assert.equal(String(fetchCalls[0].input), "https://cropcode.local/api/plugin/web-search");
...
test("WebSearch returns a configuration error when neither a script nor an LLM client is available", async () => {
  ...
  assert.equal(
    result.error,
    "WebSearch default mode requires a valid LLM configuration in ~/.cropcode/settings.json or ./.cropcode/settings.json."
  );
});
...
function createTempWorkspace(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "cropcode-web-search-"));
  tempDirs.push(dir);
  return dir;
}

...
当 AI 助手完成一轮任务后，CropCode 可以自动执行一个通知脚本，将任务结果发送到你指定的渠道（如 Slack、系统通知等）。

## 工作原理

在 `settings.json` 中配置 `notify` 字段，指向一个可执行脚本的完整路径。每次 AI 助手完成任务应答后，CropCode 会执行该脚本，并通过环境变量注入上下文信息。
...
编辑 `~/.cropcode/settings.json`，添加 `notify` 字段：
...
#!/bin/bash
# macOS 系统通知
osascript -e "display notification \"任务已${STATUS:-完成}，耗时 ${DURATION}s\" with title \"CropCode\""
```
...
notify-send "CropCode" "任务已${STATUS:-完成}，耗时 ${DURATION}s"
```

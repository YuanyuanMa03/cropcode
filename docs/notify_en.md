# CropCode Task Completion Notification

When the AI assistant finishes a round of tasks, CropCode can automatically execute a notification script to send task results to your chosen channel (Slack, system notifications, etc.).

## How It Works

Configure the `notify` field in `settings.json` with the full path to an executable script. Every time the AI assistant completes a task response, CropCode executes that script and injects context as environment variables.
...
Edit `~/.cropcode/settings.json` and add the `notify` field:
...
```json
{
  ...
  "notify": "/Users/you/.cropcode/notify-slack.sh"
}
```
...
Create `~/.cropcode/notify-slack.sh`:
...
```bash
chmod +x ~/.cropcode/notify-slack.sh
```
...
process.stdout.write(JSON.stringify({
  ...
  card: {
    header: { title: { tag: 'plain_text', content: 'CropCode: ' + process.env.TITLE + ' ' + process.env.STATUS + ' [' + process.env.DURATION + 's]' } },
...
```json
{
  ...
  "notify": "/Users/you/.cropcode/notify-feishu.sh"
}
```
...
Create `~/.cropcode/notify.sh`:

```bash
#!/bin/bash
# iTerm2 / Windows Terminal OSC 9 notification
printf '\x1b]9;CropCode: task %s (%ss)\x07' "${STATUS:-completed}" "${DURATION}"
```

```json
{
  "notify": "/Users/you/.cropcode/notify.sh"
}
```
...
echo \x1b]9;CropCode: task %STATUS% (%DURATION%s)\x07
```
...
#!/bin/bash
# macOS system notification
osascript -e "display notification \"Task ${STATUS:-completed}, took ${DURATION}s\" with title \"CropCode\""
```
...
notify-send "CropCode" "Task ${STATUS:-completed}, took ${DURATION}s"
```

```json
{
  "notify": "/home/you/.cropcode/notify.sh"
}
```
...
msg %USERNAME% "CropCode: task %STATUS% (%DURATION%s)"
```

```json
{
  "notify": "C:\\Users\\you\\.cropcode\\notify.bat"
}
```

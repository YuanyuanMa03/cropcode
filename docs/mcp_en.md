...
Once MCP is configured, CropCode can:
...
MCP tools are named in CropCode using the format `mcp__<service_name>__<tool_name>`, for example `mcp__github__search_code`.

## Configuring MCP Servers

Edit `~/.cropcode/settings.json` and add the `mcpServers` field:
...
| --------- | -------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `command` | string   | Yes      | Path or command of the MCP server executable (e.g., `npx`, `node`, `python`). When the command is `npx`, CropCode automatically prepends `-y` to the arguments. |
...
Allows CropCode to directly operate on GitHub repositories (search code, manage issues/PRs, read/write files, etc.):
...
Lets CropCode control a browser for screenshots, page interactions, etc.:
...
Enables CropCode to read and write files within a specified directory:
...
Below is a complete `~/.cropcode/settings.json` with both GitHub and Playwright MCP servers configured:
...
After configuration, start `cropcode` and type `/mcp` in the chat to view the status of all configured MCP servers and the list of tools each server provides.
...
3. Whether the terminal running `cropcode` has network access

### Tools Not Showing Up

1. Verify that the `mcpServers` field in `settings.json` is correctly formatted
2. After starting cropcode, use `/mcp` to check server status

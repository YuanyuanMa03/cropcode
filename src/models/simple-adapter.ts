...
import { prepareModelInput, type UserInput, type ModelInput } from './crop-adapter';
...
function convertWeatherToSimple(weatherData: any[], outputPath: string): void {
  const header = '*WEATHER DATA : NASA POWER\n' +
    '\n' +
    '@ INSI      LAT     LONG  ELEV   TAV   AMP REFHT WNDHT\n' +
    '  NASA  32.060   118.78    20  15.0  15.0  -99.0  -99.0\n' +
    'DATE  SRAD  TMAX  TMIN  RAIN  DEWP  WIND\n';

  const rows = weatherData.map((d: any) => {
...
function convertCultivarToSimple(cultivar: any, outputPath: string): void {
...
function convertTreatmentToSimple(treatment: any, weatherFile: string, outputPath: string): void {
  const cropMap: Record<string, string> = {
    '小麦': 'wheat',
    '水稻': 'rice',
    '玉米': 'maize',
  ...
  const content = `Species*,Exp*,Trt*,Label,weather,CO2,SowingDate,HarvestDate,SoilName,Cultivar,IrrigationTrt,MaxIntercept,InitialBio,InitialTT,InitialFsolar,NOTE\n` +
...
function convertManagementToSimple(treatment: any, outputPath: string): void {
  const cropMap: Record<string, string> = {
    '小麦': 'wheat',
    '水稻': 'rice',
    '玉米': 'maize',
  ...
  const content = `ON_Off,Species*,Exp*,Trt*,Label,Water,NOTE\n` +
...
function convertSoilToSimple(soilParams: any, outputPath: string): void {

...
export function buildExitSummaryText(input: ExitSummaryInput): string {
  ...
  const borderColor = chalk.hex("#2d8a4e");
  const titleColor = gradientString("#2d8a4e", "#8bc34a");
  const line = (text: string) => `${borderColor("│")}  ${padRight(text, contentWidth)}  ${borderColor("│")}`;

  const header = chalk.bold(titleColor("🌾 Goodbye from CropCode! Happy farming! 🌾"));
  ...
  if (hasUsage) {
    ...
    for (const { modelName, usage } of usageRows) {
      ...
      const dataRow =
        ...
        padRight(chalk.green(inputStr), colInput) +
        padRight(chalk.green(outputStr), colOutput) +
        padRight(chalk.green(cachedStr), colCached);
      rows.push(dataRow);
    }

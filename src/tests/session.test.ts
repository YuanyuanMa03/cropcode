...
test("SessionManager normalizes legacy sessions without activeTokens to zero", () => {
  const workspace = createTempDir("cropcode-legacy-active-tokens-workspace-");
...
test("SessionManager keeps usagePerModel null until response usage is available", async () => {
  const workspace = createTempDir("cropcode-null-usage-per-model-workspace-");
  const home = createTempDir("cropcode-null-usage-per-model-home-");
  setHomeDir(home);
...
test("SessionManager marks skills loaded from existing session messages", async () => {
  const workspace = createTempDir("cropcode-loaded-skills-workspace-");
...
test("SessionManager dispose disconnects MCP servers", async () => {
  const workspace = createTempDir("cropcode-mcp-dispose-workspace-");
...
test("SessionManager reports configured MCP servers as starting before initialization", () => {
  const workspace = createTempDir("cropcode-mcp-configured-workspace-");
  const manager = new SessionManager({
...
test("SessionManager reports MCP startup stderr on failure", async () => {
  const workspace = createTempDir("cropcode-mcp-failure-workspace-");
...
test(
  ...
  async () => {
    const workspace = createTempDir("cropcode-mcp-npx-workspace-");
...
test("createSession stores /init and sends the active .cropcode project AGENTS path to the LLM", async () => {
  const workspace = createTempDir("cropcode-init-cropcode-workspace-");
  const home = createTempDir("cropcode-init-cropcode-home-");
  setHomeDir(home);
  ...
  fs.writeFileSync(path.join(workspace, ".cropcode", "AGENTS.md"), "cropcode project instructions", "utf8");
  fs.writeFileSync(path.join(workspace, "AGENTS.md"), "root project instructions", "utf8");

  const manager = createSessionManager(workspace, "machine-id-init-cropcode");
...
test("createSession appends default system prompts in prefix-cache-friendly order", async () => {
  const workspace = createTempDir("cropcode-system-order-workspace-");
  const home = createTempDir("cropcode-system-order-home-");
  setHomeDir(home);
  globalThis.fetch = (async () => ({ ok: true, text: async () => "" }) as Response) as typeof fetch;
...
test("replySession stores /init and sends the active root project AGENTS path to the LLM", async () => {
  const workspace = createTempDir("cropcode-init-root-workspace-");
  const home = createTempDir("cropcode-init-root-home-");
  setHomeDir(home);
  globalThis.fetch = (async () => ({ ok: true, text: async () => "" }) as Response) as typeof fetch;
...
test("createSession stores /init and sends generate prompt when no project AGENTS file is effective", async () => {
  const workspace = createTempDir("cropcode-init-generate-workspace-");
  const home = createTempDir("cropcode-init-generate-home-");
  setHomeDir(home);
  globalThis.fetch = (async () => ({ ok: true, text: async () => "" }) as Response) as typeof fetch;

  fs.mkdirSync(path.join(home, ".cropcode"), { recursive: true });
  fs.writeFileSync(path.join(home, ".cropcode", "AGENTS.md"), "user instructions", "utf8");
...
test("createSession reports a new prompt with the machineId token", async () => {
  ...
  assert.equal(String(fetchCalls[0].input), "https://cropcode.local/api/plugin/new");
...
test(
  ...
  { skip: process.platform === "win32" },
  async () => {
    const workspace = createTempDir("cropcode-notify-success-workspace-");
    const home = createTempDir("cropcode-notify-success-home-");
    setHomeDir(home);
...
test(
  ...
  { skip: process.platform === "win32" },
  async () => {
    const workspace = createTempDir("cropcode-notify-failure-workspace-");
    const home = createTempDir("cropcode-notify-failure-home-");
    setHomeDir(home);
...
test("replySession continues without appending /continue as a user message", async () => {
  const workspace = createTempDir("cropcode-continue-workspace-");
  const home = createTempDir("cropcode-continue-home-");
  setHomeDir(home);
...
test("replySession records the current file-history branch head as checkpointHash", async (t) => {
  ...
  const workspace = createTempDir("cropcode-checkpoint-hash-workspace-");
  const home = createTempDir("cropcode-checkpoint-hash-home-");
  setHomeDir(home);
...
test("createSession initializes file-history repo and session branch", async (t) => {
  ...
  const workspace = createTempDir("cropcode-file-history-init-workspace-");
  const home = createTempDir("cropcode-file-history-init-home-");
  setHomeDir(home);
...
test("Write tool advances file-history while preserving the user prompt checkpoint", async (t) => {
  ...
  const workspace = createTempDir("cropcode-write-checkpoint-workspace-");
  const home = createTempDir("cropcode-write-checkpoint-home-");
  setHomeDir(home);
...
test("Write checkpoints restore tool-touched files outside the workspace and leave unrelated files alone", async (t) => {
  ...
  const workspace = createTempDir("cropcode-write-outside-workspace-");
  const outsideDir = createTempDir("cropcode-write-outside-target-");
  const home = createTempDir("cropcode-write-outside-home-");
  setHomeDir(home);
...
test("missing git executable does not block sessions or Write tool calls", async () => {
  const workspace = createTempDir("cropcode-no-git-write-workspace-");
  const home = createTempDir("cropcode-no-git-write-home-");
  setHomeDir(home);
...
test("restoreSessionConversation truncates messages before the selected user prompt", async () => {
  const workspace = createTempDir("cropcode-undo-conversation-workspace-");
  const home = createTempDir("cropcode-undo-conversation-home-");
  setHomeDir(home);
...
test("restoreSessionCode restores project files from the recorded Git checkpoint", async (t) => {
  ...
  const workspace = createTempDir("cropcode-undo-code-workspace-");
  const home = createTempDir("cropcode-undo-code-home-");
  setHomeDir(home);
...
test("replySession /continue runs trailing pending tool calls before requesting another response", async () => {
  const workspace = createTempDir("cropcode-continue-tool-workspace-");
  const home = createTempDir("cropcode-continue-tool-home-");
  setHomeDir(home);
...
test("replySession preserves raw session messages when a previous tool call is pending", async () => {
  const workspace = createTempDir("cropcode-pending-tool-workspace-");
  const home = createTempDir("cropcode-pending-tool-home-");
  setHomeDir(home);
...
test("LLM tool calls without ids receive generated 32 character ids", async () => {
  const workspace = createTempDir("cropcode-tool-call-id-workspace-");
  const home = createTempDir("cropcode-tool-call-id-home-");
  setHomeDir(home);
...
test("SessionManager accumulates response usage while active tokens track the latest response", async () => {
  const workspace = createTempDir("cropcode-usage-workspace-");
  const home = createTempDir("cropcode-usage-home-");
  setHomeDir(home);
...
test("SessionManager stores usage per model across model changes", async () => {
  const workspace = createTempDir("cropcode-usage-per-model-workspace-");
  const home = createTempDir("cropcode-usage-per-model-home-");
  setHomeDir(home);
...
test("SessionManager resets active tokens to latest post-compaction response usage", async () => {
  const workspace = createTempDir("cropcode-compact-usage-workspace-");
  const home = createTempDir("cropcode-compact-usage-home-");
  setHomeDir(home);
...
test("SessionManager streams chat completions and counts reasoning progress", async () => {
  const workspace = createTempDir("cropcode-stream-workspace-");
  const home = createTempDir("cropcode-stream-home-");
  setHomeDir(home);
...
test("SessionManager treats OpenAI APIUserAbortError as interrupted", async () => {
  const workspace = createTempDir("cropcode-api-abort-workspace-");
  const home = createTempDir("cropcode-api-abort-home-");
  setHomeDir(home);
...
test("SessionManager marks MCP server as failed on single failed attempt (no auto-retry)", async () => {
  const workspace = createTempDir("cropcode-mcp-fail-noworkspace-");
  const serverPath = path.join(workspace, "mcp-server-fail.cjs");
...
test("SessionManager reconnect succeeds on previously failed server", async () => {
  const workspace = createTempDir("cropcode-mcp-reconn-ok-workspace-");
...
test("SessionManager adjusts the active Bash timeout control and session metadata", async () => {
  const workspace = createTempDir("cropcode-bash-timeout-session-");
  const home = createTempDir("cropcode-bash-timeout-home-");
  setHomeDir(home);
